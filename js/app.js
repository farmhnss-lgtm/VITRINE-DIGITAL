...
function startMonitorPreviews(media){
 ...
 document.querySelectorAll('.monitor-live-preview').forEach(box=>{
  ...
  const show=()=>{
   ...
   }else if(m.type==='image'&&src){const im=new Image();im.src=src;im.alt=m.name||'Conteúdo';box.appendChild(im);schedule(()=>{if(mine===token)show()},dur)}
   else if(m.type==='text'){const d=document.createElement('div');d.className='monitor-live-text';d.textContent=m.text_content||m.name||'Texto';box.appendChild(d);schedule(()=>{if(mine===token)show()},dur)}
   else if(m.type==='web'&&src){const f=document.createElement('iframe');f.src=src;f.tabIndex=-1;f.setAttribute('aria-hidden','true');box.appendChild(f);schedule(()=>{if(mine===token)show()},dur)}
   else{box.innerHTML='<div class="monitor-placeholder">▣</div>';schedule(()=>{if(mine===token)show()},dur)}
  };show();
 });
}
function renderMonitor(screens,playlists,media,items,schedules){const grid=qs('#monitorGrid');if(!grid)return;stopMonitorPreviews();const term=(qs('#monitorSearch')?.value||'').toLowerCase(),filter=qs('#monitorFilter')?.value||'all',now=new Date();const states=screens.map(s=>({s,online:monitorIsOnline(s)}));const onlineCount=states.filter(x=>x.online).length;qs('#monitorTotal').textContent=screens.length;qs('#monitorOnline').textContent=onlineCount;qs('#monitorOffline').textContent=screens.length-onlineCount;qs('#monitorUpdated').textContent=now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});const arr=states.filter(({s,online})=>(filter==='all'||filter===(online?'online':'offline'))&&`${s.name} ${s.code} ${s.location||''}`.toLowerCase().includes(term));grid.innerHTML=arr.map(({s,online})=>{const applicable=(schedules||[]).filter(sc=>sc.active!==false&&(sc.screen_id===s.id||(s.group_id&&sc.group_id===s.group_id)));const active=applicable.filter(sc=>monitorScheduleMatches(sc,s,now)).sort((a,b)=>Number(b.screen_id===s.id)-Number(a.screen_id===s.id))[0];const outsideSchedule=applicable.length>0&&!active;const pid=outsideSchedule?null:(active?.playlist_id||s.playlist_id);const pl=playlists.find(p=>p.id===pid);const pi=items.filter(i=>i.playlist_id===pid).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0))[0];const m=media.find(x=>x.id===pi?.media_id);const preview=outsideSchedule?'<div class="monitor-blackout" title="Fora do horário de operação"></div>':monitorPreviewMarkup(s,pid,items,media);const last=s.ultima_conexao||s.last_seen||s.updated_at;return `<div class="card monitor-card ${s.orientation==='portrait'?'portrait':''}"><div class="monitor-preview">${preview}<span class="monitor-live-badge">AO VIVO</span><span class="monitor-state ${online?'online':'offline'}">${online?'● ONLINE':'● OFFLINE'}</span></div><div class="monitor-info"><div class="monitor-title">${esc(s.name)}</div><div class="monitor-meta"><b>${esc(s.code)}</b>${s.location?' · '+esc(s.location):''}<br>Última comunicação: ${esc(monitorAgo(last))}<br>${s.orientation==='portrait'?'Vertical 9:16':'Horizontal 16:9'}</div><div class="monitor-now"><b>${outsideSchedule?'Fora do horário':'Preview da playlist:'}</b> ${esc(outsideSchedule?'Tela preta':(pl?.name||m?.name||'Sem conteúdo'))}<br><span class="muted">${active?'Programação: '+esc(active.name):outsideSchedule?'Programação fechada':'Playlist: '+esc(pl?.name||'—')}</span></div><div class="monitor-actions"><button class="btn small ghost" data-screen-test="${s.id}">Abrir Player</button><button class="btn small" data-screen-edit="${s.id}">Configurar</button></div></div></div>`}).join('')||'<div class="card"><p class="muted">Nenhuma tela encontrada.</p></div>';startMonitorPreviews(media)}
...
function renderMedia(media){const term=(qs('#mediaSearch')?.value||'').toLowerCase(),type=qs('#mediaTypeFilter')?.value||'all';const arr=media.filter(m=>(type==='all'||m.type===type)&&`${m.name} ${m.type}`.toLowerCase().includes(term));qs('#mediaGrid').innerHTML=arr.map(m=>{let src=mediaSrc(m),thumb=m.type==='image'?`<img src="${esc(src)}" loading="lazy">`:m.type==='video'?`<video src="${esc(src)}" muted></video>`:m.type==='web'?`<span>🌐 WEB</span>`:`<span>▤ TEXTO</span>`;return `<div class="card media-card"><div class="thumb">${mediaSrc(m)?thumb:'Sem prévia'}</div><div class="media-info"><strong>${esc(m.name)}</strong><span class="tag">${esc(m.type)}</span> <span class="tag">${esc(m.duration||10)}s</span><div class="action-row" style="margin-top:10px"><button class="btn small" data-media-edit="${m.id}">Editar</button><button class="btn small danger" data-media-del="${m.id}">Excluir</button></div></div></div>`}).join('')||'<div class="card"><p class="muted">Nenhum conteúdo encontrado.</p></div>'}
