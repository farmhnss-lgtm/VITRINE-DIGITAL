...
function renderPlaylists(playlists,media,items){qs('#playlistGrid').innerHTML=playlists.map(p=>{const its=items.filter(i=>i.playlist_id===p.id).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));return `<div class="card playlist-card"><div class="section-head" style="margin:0"><div><h2>${esc(p.name)}</h2><small class="muted">${its.length} itens</small></div><button class="btn small" data-playlist-add="${p.id}">+ Item</button></div><div class="playlist-items" data-playlist="${p.id}">${its.map(i=>{const m=media.find(x=>x.id===i.media_id);return `<div class="drag-item" draggable="true" data-item="${i.id}"><span>☷ ${esc(m?.name||'Conteúdo removido')}</span><div class="playlist-item-actions"><button class="btn small ghost mobile-order-btn" data-item-up="${i.id}" title="Mover para cima">↑</button><button class="btn small ghost mobile-order-btn" data-item-down="${i.id}" title="Mover para baixo">↓</button><button class="btn small danger" data-item-del="${i.id}">×</button></div></div>`}).join('')||'<div class="dropzone">Arraste conteúdos para esta playlist</div>'}</div><div class="action-row" style="margin-top:10px"><button class="btn small ghost" data-playlist-preview="${p.id}">Pré-visualizar</button><button class="btn small danger" data-playlist-del="${p.id}">Excluir</button></div></div>`}).join('')||'<div class="card"><p class="muted">Crie sua primeira playlist.</p></div>';wireDrag()}
