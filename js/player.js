(function(){
  ...
  async function loadOnline(){
    ...
    root.classList.toggle('portrait',data.orientation==='portrait');
  ...
  async function heartbeat(){if(!supabase||!screen)return;try{await supabase.from('screens').update({status:'online',ultima_conexao:new Date().toISOString(),player_version:'2.0'}).eq('id',screen.id)}catch(e){setStatus('Sem sincronização');}}
